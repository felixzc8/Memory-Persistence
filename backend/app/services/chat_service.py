...
class ChatService:
    ...
    async def chat_with_memory_stream(
        ...
        try:
            ...
            logger.debug(f"LLM streaming call context - Instructions: {instructions}, Input: {user_message}")
            ...
            memory_service.memory.session_manager.add_message_to_session(session_id, "user", message, request_time)
            memory_service.memory.session_manager.add_message_to_session(session_id, "assistant", full_response, assistant_timestamp)
    ...
    async def chat_with_memory(
        ...
        try:
            ...
            logger.debug(f"LLM call context - Instructions: {instructions}, Input: {user_message}")
            ...
            memory_service.memory.session_manager.add_message_to_session(session_id, "user", message, request_time)
            memory_service.memory.session_manager.add_message_to_session(session_id, "assistant", assistant_response, assistant_timestamp)
            conversation_messages = [
                {"role": "user", "content": message},
